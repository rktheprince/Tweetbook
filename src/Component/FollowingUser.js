...
class FollowingUser extends React.Component{
    ...
    componentDidMount=()=>{
        ...
        axios.get(`http://localhost:8080/getfriend/following/${users.id}`,{headers:authHeader()})
...
handleName=(event)=>{
    ...
    unFollow=()=>{
        ...
        axios.delete(`http://localhost:8080/getfriend/unfollowing/${users.id}/${this.state.user.user_id}`,{headers:authHeader()})
        .then((result)=>{
            this.setState({
                message:result.data
            })
